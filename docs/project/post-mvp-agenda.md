...
| # | 論点 | 背景・現状 | 決めること |
| --- | --- | --- | --- |
| F-1 | **役職の制定** | 開発規約 §3.1（役割の担当者）・§3.2（意思決定者）が空欄のまま（A-3）。誰が何の責任を持つかが明文化されておらず、判断のたびに都度確認が発生している | 設置する役職と、それぞれの権限・責任範囲。担当者の割り当て。決まったら開発規約 §3.1・§3.2 に反映する（A-3 と同時に解消する） |
