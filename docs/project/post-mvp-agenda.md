...
| A-3 | **開発体制の空欄を埋める** | §3.1（役割の担当者）・§3.2（意思決定者）は**解消済み**（development-guidelines.md §3.1に反映）。残るのは §28.1 パッケージ管理ツール、§28.2 マージ方法、§28.3 ブランチ削除、§28.6 定例ミーティング | 残り4項目の記入。festival-issues.md の 0-1 に統合済み |
| A-4 | **タスク管理方法（GitHub Projects の導入検討）** | 開発規約は §4.1 で「正式な開発タスクは GitHub Issues、進捗は GitHub Projects」、§5.1 で「コードを変更する前にIssueを作成する」、§6.3 でブランチ名を `作業種別/Issue番号-作業内容` と定めているが、**実際にはIssueがほとんど使われていない**。ブランチ名にもIssue番号が入っておらず（`circle-info` など）、誰が何を持っているかがコミット履歴からしか追えない | **GitHub Projects を実際に導入するか**を決める。導入する場合はボードの構成（列・ステータス）、Issueを立てる粒度、既存の作業の登録範囲を決める。導入しない場合は §4.1・§5.1・§6.3 を実態に合わせて書き直す（規約と運用の乖離を残さない） |
...
| B-4 | **デザイン規約 §29 の未対応5件** | **Issue化済み**：HEX直書き（#4）→C-1、`display`重複（#13）→C-2、上部ナビ（#10）とハンバーガーメニュー（#11）は実装済みと判明・残作業をD-1/E-3で対応（festival-issues.md参照）。`@tailwindcss/line-clamp` 削除（#14）のみ未着手 | `@tailwindcss/line-clamp` 削除のIssue化 |
...
| E-5 | **日報の運用** | `docs/dev-report/` に numata の2026-08-02・2026-08-05 の2件のみ。他メンバーの日報は無く、運用ルールも未定 | 書く頻度・対象者・粒度 |
| E-6 | **本番環境とデプロイ** | 開発規約 §2 の「本番環境URL」「インフラ・デプロイ先」が未定。§18.1 のデプロイ担当はfestival-issues.mdの0-1に統合済み | デプロイ先（本番環境URL・インフラ）の確定。担当者は0-1で対応 |
...
| # | 論点 | 背景・現状 | 決めること |
| --- | --- | --- | --- |
| F-1 | **役職の制定** | **解消済み**（A-3と同一論点）。PM/DE/SWE/IE/WebDの5役職と担当者を決定し、役職ごとのミッション・責任範囲・主なタスクを明文化。development-guidelines.md §3.1にも反映済み | 対応不要 |
| F-2 | **Notion の運用方法の変更** | 現状の使われ方・変更したい点は**要確認**。開発タスクは §4.1 で GitHub Issues / Projects と定めており（A-4）、Notion と役割が重複しうる | Notion と GitHub で扱う情報の線引き（例：Notion は議事録・ナレッジ、GitHub は開発タスク）。ページ構成と更新の担当・頻度。A-4 と併せて決める |
