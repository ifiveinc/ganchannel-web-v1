...
判断に迷う場合は独断で決めず、DE（開発規約 §3.2）に相談する。
...
| --- | --- | --- | --- |
| フレームワーク | Next.js | **React Router v7** | 決定済み（[decisions/0002-framework-react-router.md](../decisions/0002-framework-react-router.md)）。移行の要否と時期は同ADRが持つ |
...
本規約は長期間参照されるため、時間が経つと嘘になる記述を本文に置かない。

- **時限的な記述を禁止する。** 「現時点では」「◯年◯月◯日以降」「MVP公開後」のように特定の時期を前提とした記述を、§29・§30 以外の節に書かない
- **期限つきの例外を作らない。** 例外を設ける場合は時期ではなく条件（対象・範囲）で線を引く
- 時期を伴う内容は置き場所を分ける — 是正の進捗は §29、規約の見直し時期と未決事項は §30、技術選定の移行計画は [decisions/](../decisions/) のADR
- 実装の現況（特定のファイルが今どうなっているか）も本文に書かない。§29 に集約する（§29 前書き）
...
- Tailwind標準のカラーパレット（`gray-500` `green-600` など）を新規コードで使うことの禁止。既存箇所は §29 の是正リストで置き換える
...
ダークモードには**対応しない**。対応することになった場合に備え、以下を守る。
...
- ラテン文字専用のフォント（`Inter` など）を単独で指定しない。日本語がフォールバックされ、フォント指定が意味を持たなくなる
...
なお Tailwind CSS v4 では `line-clamp-*` は標準機能であり、`@tailwindcss/line-clamp` プラグインは不要（§29 #14）。
...
- **例外はヘッダー（§19.1）のみとする。** `md:` 以上では上部ナビの項目を横に並べる必要があり、512px幅では収まらないため、ヘッダーに限り `md:max-w-full` で幅制限を解除してよい
- この例外をヘッダー以外に広げない。コンテンツ領域（`<main>`）・下部固定要素・モーダルは、画面の種類にかかわらず `max-w-lg` に従う
...
- 高さ56px（`h-14`）
- 内側は `mx-auto w-full max-w-lg px-4` を基本とし、**`md:` 以上は `md:max-w-full` で幅制限を解除する**（§6.1 が認めるただ一つの例外）。上部ナビの項目を横に並べるため
...
<header className="sticky top-0 z-20 bg-surface/95 backdrop-blur-sm">
  <div className="mx-auto flex h-14 w-full max-w-lg items-center gap-2 px-4 md:max-w-full">
...
`NavLink` の状態はTailwindのクラス関数で表現する。グローバルCSSのクラスで状態を表現しない（§26.3）。
...
デザイントークンの参照ルール（上記2〜4項）は、CSS Modules とTailwindユーティリティのどちらで書く場合も守る（[decisions/0003](../decisions/0003-css-modules-and-feature-directories.md)）。
...
新規コンポーネント、大きく作り替えるコンポーネントは CSS Modules で書くことを推奨する。
...
- 本規約 §13〜§22 のUI部品は `ui/` の共通コンポーネントとして実装する。実装済みのものがある場合、同等の見た目を個別に書き直さない
...
対応済みの項目は「状態」欄に記録する。「対象」欄のパスは規約制定時点のものであり、その後の改名・削除は「状態」欄に記す。

**実装の現況を書くのはこの表だけとする。** §1〜§28 の本文には「あるべき姿」だけを書き、特定のファイルが今どうなっているかは書かない。本文と本表の二重管理になり、片方だけが更新されて食い違うため。
...
| 7 | `app/components/layout/footer/footer.tsx` | アイコンファミリーを `react-icons/md` に統一する | §11.1 | 中 | 対応済み（2026-08-02。現 `layout/nav-items/`） |
...
| 10 | 全ページ | ヘッダー（`components/layout/header/`）が未実装。**`md:` 以上の上部ナビを含む** | §19.1 | 中 | 対応済み（`app-header.tsx` に `md:flex` の上部ナビを実装。ただし `root.tsx` が `BottomNav` を `md:` 以上でも描画したままで、`SideNav`（サイドバー）とあわせてナビが3種同時表示になっている。再度是正が必要） |
...
| 15 | `app/routes/_app._index.tsx` | コンテンツ幅に `max-w-5xl`（1024px）を使用している。§6.1 の例外はヘッダーのみで、コンテンツには適用しない | §6.1 | 中 | 未対応（対応方針が未決。`md:` 以上で2列×2行のレイアウトを組んでおり `max-w-lg` では成立しないため、単一カラムへ作り直すか、ダッシュボード用の幅を規約に新設するかの判断が要る） |
...
本規約そのものを変更する場合は、DEの承認を得たうえで、変更理由をPull Requestに記載する（開発規約 §3.2）。
...
- CSS Modules への全面移行の着手時期（§26.1・§26.3）
