...
- **時限的な記述を禁止する。** 「現時点では」「◯年◯月◯日以降」「MVP公開後」のように特定の時期を前提とした記述を、§30 以外の節に書かない
- **期限つきの例外を作らない。** 例外を設ける場合は時期ではなく条件（対象・範囲）で線を引く
- 時期を伴う内容は置き場所を分ける — 規約の見直し時期と未決事項は §30、技術選定の移行計画は [decisions/](../decisions/) のADR
- 実装の現況（特定のファイルが今どうなっているか）も本文に書かない。規約と実装が食い違う箇所を見つけたら、本文を直したうえでGitHub Issueとして起票する（development-guidelines.md §5.1）
...
- Tailwind標準のカラーパレット（`gray-500` `green-600` など）を新規コードで使うことの禁止。既存箇所を見つけたらGitHub Issueとして起票し、置き換える
...
なお Tailwind CSS v4 では `line-clamp-*` は標準機能であり、`@tailwindcss/line-clamp` プラグインは不要。
...
本規約の未決事項は`docs/project/pending-decisions.md`で一元管理する（開発規約 §26.3）。個別のリストはここには持たない。
