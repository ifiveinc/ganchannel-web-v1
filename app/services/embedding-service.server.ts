import { getSnapshotEmbeddingMetadata } from "~/services/snapshot-service.server";

// 埋め込みの固定設定（docs/chatbot-decisions.md §6）。次元数・正規化はアーキテクチャ上の
...
// クエリの埋め込みを生成する。検索カスケード第5段でのみ呼ぶ（docs/chatbot-decisions.md §6）。
...
// （docs/chatbot-decisions.md §4「index_versionが変わったキャッシュは無効化する」）。
...
// 不一致でもエラーにはせず警告のみ（docs/chatbot-decisions.md §6「不一致なら警告」）。
